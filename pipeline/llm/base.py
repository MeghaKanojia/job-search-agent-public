"""Shared rate-limiting/circuit-breaker machinery for every LLM provider.

Extracted from the incident that motivated it: a provider's retry-after can
mean two very different things -- "wait a few seconds, you're at the
per-minute limit" or "you're out for the day, don't bother." Blindly sleeping
for whatever the provider says turned one bad day with Groq into a 2+ hour
stuck job. This class treats any retry-after over `long_backoff_threshold_seconds`
as the second case: skip the wait, return None so the caller falls back, and
trip a circuit breaker so every subsequent call to THIS provider in the same
process also skips the network call instead of queuing its own long wait.
"""
...
class RateLimitedProvider:
    ...
    def _request(self, system_prompt: str, user_prompt: str) -> tuple[int, float | None, str | None]:
        """Returns (http_status, retry_after_seconds_or_None, text_or_None).
        Must not raise for ordinary HTTP error responses (that's handled via
        the returned status) -- only truly unexpected failures (network error,
        malformed response) should raise, and chat() will catch those.
        """
        raise NotImplementedError
            ...
            if status == 429:
                if retry_after and retry_after > self.long_backoff_threshold_seconds:
                    ...
                    logger.warning(
                        ...
                        "not a per-minute window. Skipping %s for the rest of this run.",
                        self.name,
                        retry_after,
                        self.name,
                    )
